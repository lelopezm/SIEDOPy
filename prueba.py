...
def run_modelo_logistico_interface():
    try:
        # Ruta al script que deseas ejecutar
        script_path = os.path.join(sys.path[0], 'ScriptsPython', 'CodigosPVI', 'InterfazModeloLogistico.py')
        ...
        if os.path.isfile(script_path):
            ...
            messagebox.showerror("Error", "El archivo InterfazModeloLogistico.py no se encontró.")
    except subprocess.CalledProcessError as e:
        messagebox.showerror("Error", f"Hubo un error al ejecutar InterfazModeloLogistico.py: {e}")
...
root.geometry("400x400")  # Tamaño aumentado para dar más espacio
...
modelo_logistico_button = ttk.Button(button_frame, text="Modelo Logístico", command=run_modelo_logistico_interface)
modelo_logistico_button.grid(row=4, column=0, columnspan=3, sticky='ew', padx=10, pady=10)

salir_button = ttk.Button(button_frame, text="Salir", command=salir_de_la_aplicacion)
salir_button.grid(row=5, column=0, columnspan=3, sticky='ew', padx=10, pady=10)
